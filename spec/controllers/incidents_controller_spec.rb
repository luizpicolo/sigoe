...
RSpec.describe IncidentsController, type: :controller do
  ...
  # let(:course) { FactoryBot.attributes_for(:course).merge(polo_id: user.polo.id) }
  # let(:incident_attributes) { FactoryBot.attributes_for(:incident, visibility: 'public', user_id: user.id, course: course) }
